...
- **Deploy**: push to `master` triggers `.github/workflows/pages.yml`, which copies the actual web-facing files (`index.html`/`style.css`/`data.js`/`app.js`/`manifest.json`/`sw.js`/`icons/`, not `README.md`, `prompt.md`, or `.claude/`) into a Pages artifact and deploys via `actions/deploy-pages`. **When adding a new file the browser needs to fetch, add it to that workflow's "Prepare site files" step too** — it's an explicit copy list, not a directory mirror, and a file existing in the repo does not mean it's deployed (manifest.json/sw.js/icons/ 404'd on first PWA deploy because of exactly this). That final deploy step also intermittently fails with a generic "Deployment failed, try again later" from GitHub's side (the build itself always succeeds) — `gh run rerun <run-id> --repo saitekii/grocery-generator` resolves it, usually within 1-2 retries.
...
**State** (`app.js`, `localStorage` key `groceryGenerator:v4`): `{ activeWeekStart, weeks: { [mondayISODate]: { days, groceryIds, checked } }, pantryStock }`. Weeks are keyed by their Monday date and kept indefinitely (a year of weekly plans is well under a megabyte) — there is no expiry or pruning. Each day holds `MEALS_PER_DAY` slots (`{ mealId, prefs: {style, highProtein, cheap} }`); per-meal prefs, not global toggles, drive filtering. `pantryStock` (`{id: bool}`) is the one piece of state that's global rather than per-week — items marked "already have" stay off every week's rendered grocery list until un-marked, unlike `week.checked` which is per-week shopping progress and never affects what's shown. Bump the `STORAGE_KEY` version suffix on any breaking change to this shape rather than writing migration code — this is a single-user local app, so old saved state is simply abandoned, not migrated. This has happened several times already (v1→v4); `pantryStock` itself was added additively (defaulted in `init()` if missing) without a version bump since it doesn't break existing saved weeks.
